...
def edit_profile(request):
    if request.method == 'POST': 
            ...
            print(dir(request.user))
            profiles = student_profiles(request.POST, instance=request.user.student_profile)
            if userform.is_valid() and profiles.is_valid():
                ...
                messages.error(request, "Please Correct the following Error Below"+str(profiles))
    else:
